...
class OrderManager {
	...
	/**
	 * Get date range arguments for order query.
	 *
	 * @param string $date_filter Date filter value (today, week, month, quarter, year).
	 * @return array Date range arguments for wc_get_orders().
	 */
	private function get_date_range_args( $date_filter ) {
		$args = array();
		$now  = current_time( 'timestamp' );

		switch ( $date_filter ) {
			case 'today':
				$start_date         = gmdate( 'Y-m-d H:i:s', strtotime( 'today', $now ) );
				$end_date           = gmdate( 'Y-m-d H:i:s', strtotime( 'tomorrow', $now ) - 1 );
				$args['meta_query'] = array(
					'relation' => 'AND',
					array(
						'key'     => '_date_created',
			...
			case 'week':
				$start_date         = gmdate( 'Y-m-d H:i:s', strtotime( '-7 days', $now ) );
				$args['meta_query'] = array(
					array(
						'key'     => '_date_created',
						'value'   => $start_date,
			...
			case 'month':
				$start_date         = gmdate( 'Y-m-d H:i:s', strtotime( '-30 days', $now ) );
				$args['meta_query'] = array(
					array(
						'key'     => '_date_created',
						'value'   => $start_date,
			...
			case 'quarter':
				$start_date         = gmdate( 'Y-m-d H:i:s', strtotime( '-90 days', $now ) );
				$args['meta_query'] = array(
					array(
						'key'     => '_date_created',
						'value'   => $start_date,
			...
			case 'year':
				$start_date         = gmdate( 'Y-m-d H:i:s', strtotime( '-1 year', $now ) );
				$args['meta_query'] = array(
					array(
						'key'     => '_date_created',
						'value'   => $start_date,
