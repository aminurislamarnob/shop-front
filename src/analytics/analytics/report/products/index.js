	...
	getChartMeta() {
		...
		const label         =
			isSingleProductView && isSingleProductVariable
				? __( '%d variations', 'storesuite' )
				: __( '%d products', 'storesuite' );

		return { compareObject, itemsLabel: label, mode };
	}
...
export default compose(
	withSelect( ( select, props ) => {
		...
		const isSingleProductView =
			! query.search && query.products && query.products.split( ',' ).length === 1;

		const { getItems, isResolving, getItemsError } = select( ITEMS_STORE_NAME );

		if ( isRequesting ) {
			return { query: { ...query }, isSingleProductView, isRequesting };
		}
		...
		if ( isSingleProductView ) {
			...
			return {
				query:                { ...query, 'is-variable': isVariable },
				isSingleProductView,
		...
		return { query, isSingleProductView };
	} )
)( ProductsReport );
