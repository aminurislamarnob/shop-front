...
const OrderSettings = () => {
		...
		const fetchSettings = async () => {
			try {
				...
				if (response.storesuite_dashboard_page_id) {
					setProductPerPage(response.storesuite_dashboard_page_id);
				}
	...
	const handleSubmit = async (event) => {
		...
		setIsLoading(true);
		try {
			const { storesuite_product_per_page } = await apiFetch({
				path: '/storesuite/v1/settings',
				method: 'POST',
				data: {
			...
			setProductPerPage(storesuite_product_per_page);
	...
	return (
		<div>
			...
			<div className="settings-header">
				...
				<h2>{__('Order Settings', 'storesuite')}</h2>
			</div>
			{message && (
				<Notice
			...
			<form onSubmit={handleSubmit}>
				<Card>
					<CardBody>
						<TextControl
							label={__('Product per page', 'storesuite')}
