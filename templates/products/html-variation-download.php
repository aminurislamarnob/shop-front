<?php
/**
 * Template used to render an individual row within a variation's downloadable files table.
 *
 * @package StoreSuite
 *
 * @var int    $loop  Variation index for array-named inputs.
 * @var array  $file  Download data ( name, file ).
 * @var string $key   Download key/hash.
 */
...
<tr>
	...
	<td class="file_name">
		<input type="text" class="storesuite-form-control input_text" placeholder="<?php esc_attr_e( 'File name', 'storesuite' ); ?>" name="_wc_variation_file_names[<?php echo esc_attr( $loop ); ?>][]" value="<?php echo esc_attr( $file['name'] ); ?>" />
		<input type="hidden" name="_wc_variation_file_hashes[<?php echo esc_attr( $loop ); ?>][]" value="<?php echo esc_attr( $key ); ?>" />
	</td>
	<td class="file_url">
		<input type="text" class="storesuite-form-control input_text storesuite-downloadable-file-url-input" placeholder="<?php esc_attr_e( 'https://', 'storesuite' ); ?>" name="_wc_variation_file_urls[<?php echo esc_attr( $loop ); ?>][]" value="<?php echo esc_attr( $file['file'] ); ?>" />
	</td>
	<td class="file_url_choose" width="15%" colspan="2">
		...
		<a href="#" class="storesuite-delete-file" aria-label="<?php esc_attr_e( 'Delete', 'storesuite' ); ?>" title="<?php esc_attr_e( 'Delete', 'storesuite' ); ?>">
			<svg width="16" height="16" fill="currentColor" aria-hidden="true" focusable="false"><use href="#storesuite-icon-delete"></use></svg>
		</a>
	</td>
</tr>
